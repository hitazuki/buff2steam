...
from .clients import BuffClient, C5Client, C5ClientError, SteamClient
from .config import DEFAULT_CONFIG, load_config
from .currency import CurrencyConverter
from .matching import TransactionMatcher
from .migration import migrate_legacy_cache
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "command",
        choices=("sync", "refresh", "build", "check-login", "view", "cookies"),
        help=(
            "执行动作：sync 增量抓取后聚合；refresh 全量抓取后聚合；"
            "build 使用本地数据聚合并导出；check-login 仅校验 Cookie；"
            "view 查看已导出的 CSV；cookies 从本地 Chrome 提取 Cookie"
        ),
    )
    parser.add_argument(
        "view_path",
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "cookies":
        if args.view_path is not None or args.platforms or args.no_export or args.open_html:
            parser.error("cookies 不接受报告或平台参数")
        from .cookies import main as cookies_main
        cookies_main(Path(args.config))
        return
    ...
    data_dir = Path(settings.get("data_dir", "./data/profit"))
    output_dir = settings.get("output_dir", "./output/profit")
    cache_ttl = settings.get("cache_ttl_hours", 6)
    if Path(args.config) == DEFAULT_CONFIG and data_dir == Path("data/profit"):
        migrate_legacy_cache(data_dir)
